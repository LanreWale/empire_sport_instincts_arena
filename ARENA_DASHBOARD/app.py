    ...
    with st.sidebar:
        ...
        try:
            for s in data.router.get_provider_status():
                color = "#00ff88" if "ONLINE" in s["status"] or "🟢" in s["status"] else "#888"
                st.markdown(
        ...
        if chosen_sport != prev_sport:
            ...
            _clear_all_caches()        # ← Critical: clear stale data on sport change
            st.rerun()
        ...
        # ← Critical: when league changes, clear caches so fresh filtered data loads
...
# ── Cached fetchers (NOTE: no session_state access inside cached funcs) ──────
...
def _fetch_matches(sport: str, league_id: str, status: str) -> pd.DataFrame:
    try:
        if status == "LIVE":
            df = _fetch_live(sport, league_id)
        elif status in ("UPCOMING", "SCHEDULED"):
            df = _fetch_upcoming(sport)
        elif status == "FINISHED":
            df = pd.DataFrame()
        else:  # ALL
            live_df     = _fetch_live(sport, league_id)
            upcoming_df = _fetch_upcoming(sport)
...
def _get_league_name(sport: str, league_id: str) -> str:
    ...
    for lg in data.get_all_leagues(sport):
        if str(lg.get("id", "")) == str(league_id):
            return lg.get("name", league_id)
    return league_id


def _apply_league_filter(df: pd.DataFrame, sport: str, league_id: str):
    """
    Strict league filtering. Returns (filtered_df, league_name, filter_status)
    filter_status: 'all' | 'exact' | 'partial' | 'none'
    """
    league_name = _get_league_name(sport, league_id)

    if league_id == "ALL" or df.empty:
        return df, league_name, "all"

    # 1) Best: exact LEAGUE_ID match (if data layer provides it)
    if "LEAGUE_ID" in df.columns:
        filtered = df[df["LEAGUE_ID"].astype(str) == str(league_id)]
        if not filtered.empty:
            return filtered, league_name, "exact"

    # 2) Exact league name match (case-insensitive)
    if "LEAGUE" in df.columns:
        filtered = df[df["LEAGUE"].astype(str).str.strip().str.lower()
                      == league_name.strip().lower()]
        if not filtered.empty:
            return filtered, league_name, "exact"

        # 3) Conservative partial match — full league name as substring
        filtered = df[df["LEAGUE"].astype(str).str.contains(
            league_name, case=False, na=False, regex=False)]
        if not filtered.empty:
            return filtered, league_name, "partial"

    # 4) Nothing matched → return EMPTY (never silently show wrong leagues!)
...
def render_prediction_card(pred: MatchPrediction):
    ...
    value_badge = (
        '<span class="value-badge">💰 VALUE BET</span>'
    ...
    with f1:
        st.markdown(
            '<div style="font-family:Orbitron;font-size:.7rem;color:#D4AF37;margin-bottom:6px;">✅ KEY FACTORS</div>',
            unsafe_allow_html=True,
        )
    ...
    with f2:
        st.markdown(
            '<div style="font-family:Orbitron;font-size:.7rem;color:#ff6b6b;margin-bottom:6px;">⚠️ RISK FACTORS</div>',
            unsafe_allow_html=True,
        )
...
def render_match_cards(matches_df: pd.DataFrame, sport: str):
    if matches_df is None or matches_df.empty:
        st.info(f"📡 No {sport} matches found for this selection. Try a different league or status filter.")
        return
...
def render_match_detail():
    ...
    if st.button("← Back to Match List"):
        for k in ("selected_match_id", "selected_match_row",
                  "selected_match_home", "selected_match_away", "selected_match_sport"):
...
# ══════════════════════════════════════════════════════════════════════════════
# ARENA — WITH STRICT LEAGUE FILTERING (THE CORE FIX)
# ══════════════════════════════════════════════════════════════════════════════
def render_arena(sport: str, league_id: str, status: str):
    ...
    st.markdown(
        ...
        f'margin-bottom:10px;">📡 {provider} | Real-time data</div>',
        unsafe_allow_html=True,
    )

    with st.spinner(f"📡 Fetching {sport} matches from global APIs..."):
    ...
    if filter_status == "exact":
        st.success(f"✅ Showing {len(df)} matches — **{league_name}**")
    elif filter_status == "partial":
        st.info(f"ℹ️ Partial name match: showing {len(df)} matches for **{league_name}**")
    elif filter_status == "none":
        st.warning(
            f"⚠️ No **{league_name}** matches in the current {status} window. "
            f"This league may be off-season or have no fixtures in the next 7 days. "
            f"Try a different status filter or select 'All Events'."
        )

    # Optional debug panel
    if st.session_state.get("show_debug"):
        with st.expander("🔧 DEBUG INFO", expanded=False):
            st.write(f"**League ID:** {league_id} | **Name:** {league_name} | **Filter:** {filter_status}")
            st.write(f"**Shape:** {df.shape} | **Columns:** {list(df.columns) if not df.empty else '—'}")
            raw = _fetch_matches(sport, "ALL", status)
            if not raw.empty and "LEAGUE" in raw.columns:
                st.write("**All leagues returned by API:**")
...
def render_predictions(sport: str, league_id: str, status: str):
    ...
    if df.empty:
        st.info(f"No {league_name} matches to analyse. Adjust the league/status filter in the sidebar.")
        return
    ...
    with col_info:
        st.markdown(
            ...
            f'🔍 Will scan {min(len(df), 20)} {league_name} matches | picks ≥ 65% confidence</div>',
            unsafe_allow_html=True,
        )
    ...
    if run_scan or "batch_result" in st.session_state:
        ...
        if result and isinstance(result, BulkScanResult):
            st.markdown(
                ...
                f'{len(result.value_bets)} value bets | at {result.scan_time}</div>',
                unsafe_allow_html=True,
            )
            ...
            if result.high_conf_picks:
                ...
                for pick in result.high_conf_picks:
                    ...
                    st.markdown(
                        ...
                        f'<span style="color:{color};font-family:Orbitron;font-size:.85rem;font-weight:700;">'
                        f'{conf}% {pick.get("value_rating","")}</span></div>'
            ...
            if result.value_bets:
                ...
                for vb in result.value_bets:
                    st.markdown(
                        ...
                        f'Edge: {vb.get("edge","")} {vb.get("rating","")}</span></div></div>',
                        unsafe_allow_html=True,
                    )
        else:
            st.warning("Scanner returned no picks above the confidence threshold.")
        ...
        with col_info:
            ...
            if has_pred and hasattr(cached_pred, "confidence"):
                ...
                badge = (
                    ...
                    f'border:1px solid {cc};border-radius:8px;padding:2px 8px;">🧠 {cached_pred.confidence}%</span>'
                )
            st.markdown(
...
def render_analytics():
    ...
    st.markdown(
        ...
        f'<div style="font-family:Orbitron;font-size:.9rem;color:{ai_col};">🧠 CLAUDE AI ENGINE STATUS: {ai_stat}</div>'
        ...
        f'Confidence thresholds: HIGH ≥70% | MEDIUM ≥55% | LOW below 55%</div></div>',
        unsafe_allow_html=True,
    )
    ...
    if pred_log:
        ...
        st.dataframe(pd.DataFrame(pred_log), use_container_width=True, hide_index=True, height=300)
    else:
    ...
    try:
        for s in data.router.get_provider_status():
            color = "#00ff88" if ("ONLINE" in s["status"] or "🟢" in s["status"]) else "#888"
            st.markdown(
                f'<div class="stat-row"><span class="stat-label">{s["name"]}</span>'
    ...
    st.markdown(
        f'<div class="stat-row"><span class="stat-label">Claude AI (Anthropic)</span>'
