...
# ══════════════════════════════════════════════════════════════════════════════
# SPORT CONFIG - CHANGED FROM "Soccer" TO "Football"
# ══════════════════════════════════════════════════════════════════════════════
SPORT_OPTIONS = {
...
def render_arena(sport: str, league_id: str, status: str):
    ...
    st.markdown(
        ...
        f'margin-bottom:10px;">📡 {provider}</div>',
        unsafe_allow_html=True,
    )

    spinner_msg = f"📡 Fetching {sport} matches..."
        ...
        with st.spinner("🧠 Claude AI scanning all matches for value..."):
            ...
            if result and result.high_conf_picks:
                for pick in result.high_conf_picks[:5]:
                    st.markdown(f"**{pick.get('home_team')} vs {pick.get('away_team')}** - {pick.get('confidence')}%")
